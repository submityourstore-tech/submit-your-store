...
export async function getPublicBusinesses(query: PublicBusinessQuery = {}): Promise<Business[]> {
  const businesses = await allBusinesses();
  return businesses.filter((business) => matchesPublicQuery(business, query));
}
...
export async function getActiveVerticalStats(): Promise<VerticalStat[]> {
  const businesses = (await allBusinesses()).filter(isActiveBusiness);
  ...
  for (const vertical of CATEGORY_REGISTRY) {
    ...
    const navSlug =
      vertical.slug === "home-services" && verticalNavLabel(vertical.slug, businesses) === "HVAC Texas"
        ? LEGACY_HVAC_VERTICAL
        : vertical.slug;
...
export async function getActiveSubcategoryStats(verticalSlug: string): Promise<CategoryStat[]> {
...
export async function getActiveSearchTags(limit = 6): Promise<CategoryStat[]> {
  const activeVerticals = await getActiveVerticalStats();
