...
export function HvacListing({
  ...
  const totalPages = !groupByCity ? Math.max(1, Math.ceil(results.length / PAGE_SIZE)) : 1;
  const currentPage = !groupByCity ? Math.min(initialPage, totalPages) : 1;
  const paginatedResults = useMemo(() => {
    if (groupByCity) return results;
    const start = (currentPage - 1) * PAGE_SIZE;
    return results.slice(start, start + PAGE_SIZE);
  }, [groupByCity, results, currentPage]);
  ...
  function pushState(next: {
    ...
    if (!groupByCity && page > 1) params.set("page", String(page));
  ...
  function handleFilterChange(value: FilterOption) {
    ...
    setSort(nextSort);
    pushState({ filter: nextFilter, sort: nextSort, page: 1 });
  }

  function handleCategoryChange(value: string) {
    setCategory(value);
    pushState({ category: value, page: 1 });
  }
    ...
    <>
      ...
      <div className="mx-auto max-w-6xl px-4 py-6">
        {results.length === 0 ? (
          ...
          <>
            <div className="flex flex-col gap-4">{renderCards(paginatedResults)}</div>
            {!groupByCity && results.length > PAGE_SIZE && (
              <ListingPagination
                basePath={basePath}
