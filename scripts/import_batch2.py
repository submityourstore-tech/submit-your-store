...
from import_hvac_faqs import apply_csv_row, norm_name
...
def parse_city_state(address: str | None) -> tuple[str, str]:
    if not address or is_missing(address):
        return "Dallas", "TX"
    match = re.search(
        r",\s*([^,]+?),\s*(TX|Texas|[A-Z]{2})\s+\d{5}(?:,\s*United States)?",
        address,
        re.I,
    )
    if match:
        city = match.group(1).strip()
        state_raw = match.group(2).strip().upper()
        state = "TX" if state_raw == "TEXAS" else state_raw
        return city, state
    return "Dallas", "TX"
...
def process_row(raw: dict, used_ids: set[str]) -> dict:
    ...
    if is_missing(email) or email in PLACEHOLDER_EMAILS or "sentry" in email or "wixpress" in email:
        email = None
    ...
    if not is_missing(raw.get("city", "")):
        city = raw.get("city", city).strip() or city
    if not is_missing(raw.get("state", "")):
        state = raw.get("state", state).strip() or state
...
def main() -> None:
    ...
    used_gbp = {
        b.get("googleMapsUrl", "").strip()
        for b in existing
        if b.get("googleMapsUrl")
    }
    used_names = {norm_name(b["name"]) for b in existing}

    with csv_path.open(encoding="utf-8", newline="") as f:
        raw_rows = list(csv.DictReader(f))

    imported: list[dict] = []
    stats = {"logo": 0, "gallery": 0, "no_images": 0, "skipped": 0}

    for i, raw in enumerate(raw_rows, 1):
        name = raw["business_name"].strip()
        gbp = raw.get("gbp_url", "").strip()
        if gbp in used_gbp or norm_name(name) in used_names:
            stats["skipped"] += 1
            print(f"[{i}/{len(raw_rows)}] SKIP duplicate: {name[:55]}...")
            continue

        print(f"[{i}/{len(raw_rows)}] {name[:55]}...")
    ...
    print(
        f"Skipped duplicates: {stats['skipped']}, logos: {stats['logo']}, "
        f"with gallery: {stats['gallery']}, no images: {stats['no_images']}"
    )
