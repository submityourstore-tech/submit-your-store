...
import { getUserVote } from "@/lib/business-votes.server";
...
export default async function BlogPostPage({ params }: PageProps) {
  ...
  const businesses = getTopBusinessesForBlog(post.city, post.state);

  const comments = getCommentsForBlog(slug);
      ...
      <div className="mx-auto max-w-3xl px-4 py-8 pb-12">
        ...
        <article className="content-page mt-8">
          ...
          <p className="mt-6 text-xs text-[#717171]">
            ...
            ⭐ Rankings combine community upvotes with overall Google ratings and review volume. Each company links to

            its full profile with logo, hours, and customer reviews.
          ...
          {businesses.length === 0 ? (
            ...
            <ol className="mt-6 space-y-4">
              ...
              {businesses.map((business, index) => {
                ...
                const stats = getBusinessVoteStats(business.id);

                const userVote = user ? getUserVote(business.id, user.id) : null;
