...
export async function GET(request: Request, context: RouteContext) {
  ...
  if (!token || !(await validateManageToken(id, token))) {
...
export async function PUT(request: Request, context: RouteContext) {
  const { id } = await context.params;
  const token = await getToken(request, id);
  const session = token ? await validateManageToken(id, token) : null;
  if (!session) {
    return NextResponse.json({ error: "Unauthorized." }, { status: 401 });
  }
  ...
  const updated = await updateBusiness(id, patch as Partial<import("@/types/business").Business>);
  if (!updated) {
    return NextResponse.json({ error: "Business not found." }, { status: 404 });
  }
