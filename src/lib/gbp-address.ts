...
export function extractGooglePlaceId(url: string): string | null {
  ...
  const fromParam = decoded.match(/[?&](?:query_)?place_id=([^&]+)/i)?.[1];
...
export async function fetchGbpAddressFromPlacesApi(
  ...
  const res = await fetch(`https://places.googleapis.com/v1/places/${encodeURIComponent(placeId)}`, {
    headers: {
      ...
      "X-Goog-Api-Key": key,
      "X-Goog-FieldMask":
        "formattedAddress,location,displayName,addressComponents,nationalPhoneNumber,internationalPhoneNumber",
    },
  ...
  const phone = data.nationalPhoneNumber ?? data.internationalPhoneNumber ?? undefined;
